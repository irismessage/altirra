...
ATDebuggerLogChannel g_ATLCEEPROMRead(false, false, "EEPROMR", "EEPROM reads");
ATDebuggerLogChannel g_ATLCEEPROMWrite(false, false, "EEPROMW", "EEPROM writes");
ATDebuggerLogChannel g_ATLCEEPROM(false, false, "EEPROM", "EEPROM commands");
...
void ATEEPROMEmulator::DumpStatus() {
	ATConsoleWrite("25AA02A status:\n");
	ATConsolePrintf("  Output state:     %d\n", mbState);
	ATConsolePrintf("  Current register: $%02x (%s)\n", mAddress & 0x7f, mAddress & 0x80 ? "write" : "read");
	ATConsolePrintf("  Phase:            %s bit %u\n", mPhase & 8 ? "data" : "address", ~mPhase & 7);
	ATConsoleWrite("\n");
	ATConsoleWrite("NVRAM:\n");

	for(int offset = 0; offset < 0x100; offset += 0x10) {
		ATConsolePrintf("%02X: %02X %02X %02X %02X %02X %02X %02X %02X-%02X %02X %02X %02X %02X %02X %02X %02X\n"
			, offset + 0x20
			, mMemory[offset + 0]
			, mMemory[offset + 1]
			, mMemory[offset + 2]
