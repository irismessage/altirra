...
class ATUIDialogDeviceSX212 : public VDDialogFrameW32 {
public:
	ATUIDialogDeviceSX212(ATPropertySet& props);
...
const wchar_t *const ATUIDialogDeviceSX212::kTerminalTypes[]={
...
const wchar_t *const ATUIDialogDeviceSX212::kSioEmuModes[]={
	L"None - Emulated R: handler only",
	L"Full - SIO protocol",
};

ATUIDialogDeviceSX212::ATUIDialogDeviceSX212(ATPropertySet& props)
	: VDDialogFrameW32(IDD_DEVICE_SX212)
	, mPropSet(props)
{
}

bool ATUIDialogDeviceSX212::OnLoaded() {
...
void ATUIDialogDeviceSX212::OnDataExchange(bool write) {
	if (write) {
		mPropSet.Clear();
...
bool ATUIDialogDeviceSX212::OnCommand(uint32 id, uint32 extcode) {
	if (id == IDC_TELNET) {
...
void ATUIDialogDeviceSX212::UpdateEnables() {
...
bool ATUIConfDevSX212(VDGUIHandle hParent, ATPropertySet& props) {
	ATUIDialogDeviceSX212 dlg(props);
